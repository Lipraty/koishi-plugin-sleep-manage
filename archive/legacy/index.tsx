import { $, Context, Keys, Schema, Session, SessionError, observe } from 'koishi'
import { } from '@koishijs/plugin-help'
import * as Commander from './command'
import { Periods, SleepManage } from './types'
...
export function apply(ctx: Context, config: SleepManage.Config) {
  ...
  ctx.on('dispose', () => {
    userGagInterval.forEach(v => clearInterval(v))
  })
  ...
  ctx.middleware(async ({ content, isDirect, platform, guildId, user, $sleep, sleepField, execute }: Session<SleepManage.User | 'id'>, next) => {
    ...
    const triggerWithTimezone = triggerZone(nowTime, user[SleepManage.User.TimeZone], config)

    let $triggerPeriod: Periods

    if (triggerWithTimezone === -1) {
      return await next()
    } else if (triggerWithTimezone === 1 && config.morningWord.includes(content)) {
      $triggerPeriod = Periods.MORNING
    } else if (triggerWithTimezone === 0 && config.eveningWord.includes(content)) {
      $triggerPeriod = Periods.EVENING
    } else {
      return await next()
    }
 ...
 * @param timezone
 * @param config
 * @returns 0: evening, 1: morning, -1: other
 */
export function triggerZone(timespace: number, timezone: number, config: SleepManage.Config): 0 | 1 | -1 {
  if (timezone > 12 || timezone < -12)
    throw new SessionError(['sleep.timezone.invalid'])
  const { morningSpan } = config
  const time = getTimeByTZ(timezone, timespace).getHours()
  return (time >= morningSpan[0] && time <= morningSpan[1]) ? 1 : (time >= config.eveningSpan[0] && time <= config.eveningSpan[1]) ? 0 : -1
}
